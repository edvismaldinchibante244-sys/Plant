...
include_once '../config/restaurante_status_helper.php';
...
<head>
    ...
    <title>Administração - Sistema de Restaurantes</title>`r`n    <link rel="icon" href="favicon.ico" type="image/x-icon">
...
<body>
    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <main class="main-content col-md-9 ms-sm-auto col-lg-10">
                ...
                <?php if (!$can_manage_restaurants && !$can_manage_users && !$can_access_plans): ?>
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select" name="plano" required>
                                    ...
                                    <option value="EMPRESARIAL">Empresarial (3.000 MZN/mês)</option>
                                </select>
                            </div>
                            <div class="col-md-6">
    ...
    <script>
        ...
        function restoreStateFromUrl() {
            ...
            if (secao && ['restaurantes', 'usuarios', 'planos'].includes(secao)) {
                secaoAtual = secao;
            }
        ...
        function reconciliarCompraProcessada(compraId, acao, modalAlertId, modalElementId) {
            ...
            return obterCompraPorId(compraId)
                .then(compra => {
                    if (compra && String(compra.status || '').toUpperCase() !== 'PENDENTE') {
        ...
        function aplicarFiltroCompras(resetPage = true) {
            ...
            tableState.compras.filtered = tableState.compras.all.filter(c => {
                ...
                const statusCompra = String(c.status || '').toUpperCase();
                ...
                const hay = [c.restaurante_nome, c.plano_atual, c.plano_novo, c.status, c.metodo_pagamento, c.ciclo].map(normalizeText).join(' ');
                return hay.includes(query);
            });
            if (resetPage) {
                tableState.compras.page = 1;
        ...
        function atualizarTabelaCompras(compras) {
            ...
            if (compras.length === 0) {
                tbody.innerHTML = '<tr><td colspan="11" class="text-center py-4 text-muted">Nenhuma compra encontrada</td></tr>';
                return;
            }
            ...
            compras.forEach(c => {
                ...
                let botoes = '';
                if (c.status === 'PENDENTE' && hasPermission('approve_plans')) {
        ...
        function atualizarEstatisticasCompras(compras) {
            ...
            const aprovados = compras.filter(c => c.status === 'APROVADO').length;
        ...
        function abrirComprovativo(pathEncoded) {
            ...
            if (lower.endsWith('.pdf')) {
                body.innerHTML = `<iframe src="${escapeHtml(path)}" style="width:100%;height:70vh;border:0;border-radius:8px;"></iframe>`;
            } else {
                body.innerHTML = `<div class="text-center"><img src="${escapeHtml(path)}" alt="Comprovativo" style="max-width:100%;max-height:70vh;border-radius:10px;"></div>`;
            }
                    ...
                    if (data.success) {
                        ...
                        if (passwordSetupUrl) {
                            ...
                            if (isLocalEnvironment()) {
                                supportMessages.push('Ambiente local detectado: se precisar, abra a definicao de senha manualmente pelo botao ao lado.');
                            }
                            const supportMessage = supportMessages.length ?
                                supportMessages.join(' ') :
                                'Use o botao ao lado para abrir a definicao de senha do administrador.';
